...
//		List<CommentVO> list = scv.getList(bno);
	...
	//@PutMapping: 이 애노테이션은 HTTP PUT 요청을 처리하는 컨트롤러 메소드로 표시합니다.
	//value="/{cno}": 이 부분은 엔드포인트의 URL 패턴을 설정합니다. {cno}는 경로 변수이며, 
	//클라이언트가 요청 URL의 일부로 전달하는 값을 나타냅니다. 예를 들어, 
	//'/123' 경로로 요청이 온다면 {cno} 변수에 123이 들어갑니다.
	//consumes = "application/json": 이 부분은 요청이 'application/json' 미디어 유형을 소비한다는 것을 나타냅니다. 즉, 클라이언트는 요청 본문에 JSON 데이터를 제공해야 합니다.
	//produces = MediaType.TEXT_PLAIN_VALUE: 이 부분은 이 메소드가 텍스트 평문 문자열을 생성하여 응답으로 보낸다는 것을 나타냅니다.
